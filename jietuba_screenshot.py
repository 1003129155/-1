...
def _load_long_stitch_engine():
    ...
    raw_engine = settings.value('screenshot/long_stitch_engine', 'hash_rust', type=str)
    engine = normalize_engine_value(raw_engine)
    
    # 🆕 如果检测到auto或rust，强制切换为hash_rust
    if engine in ('auto', 'rust'):
        print(f"⚠️ 检测到已禁用的引擎 {engine}，自动切换为 hash_rust")
        engine = 'hash_rust'
...
class ScrollCaptureWindow(QWidget):
    ...
    def _setup_floating_toolbar(self):
        ...
        # 先显示工具栏以获取正确的尺寸
        self.toolbar.show()
        self.toolbar.adjustSize()  # 调整到合适的大小
        
        # 计算工具栏位置（默认在截图区域上方居中）
        toolbar_x = self.x() + (self.width() - self.toolbar.width()) // 2
        toolbar_y = self.y() - self.toolbar.height() - 10  # 距离截图区域上方10像素
        ...
        if toolbar_y < screen_geometry.top():
            # 如果超出顶部，放在截图区域下方
